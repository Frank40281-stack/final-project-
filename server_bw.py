"""Alpha local development server (Business Weekly Style Theme on Port 5505)."""
...
class AlphaBWRequestHandler(SimpleHTTPRequestHandler):
...
if __name__ == "__main__":
    ...
    handler = lambda *args, **kwargs: AlphaBWRequestHandler(
        *args, directory=str(project_root), **kwargs
    )
    server = ThreadingHTTPServer((HOST, PORT), handler)
    print(f"Alpha BW local server: http://{HOST}:{PORT}")
