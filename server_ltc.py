"""Alpha local development server (1966 LTC Style Theme on Port 5506)."""
...
class AlphaLTCRequestHandler(SimpleHTTPRequestHandler):
...
if __name__ == "__main__":
    ...
    handler = lambda *args, **kwargs: AlphaLTCRequestHandler(
        *args, directory=str(project_root), **kwargs
    )
    server = ThreadingHTTPServer((HOST, PORT), handler)
    print(f"Alpha LTC local server: http://{HOST}:{PORT}")
