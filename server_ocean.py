"""Alpha local development server (Deep Ocean Navy & Gold Spotlight Theme on Port 5502)."""
...
class AlphaOceanRequestHandler(SimpleHTTPRequestHandler):
...
if __name__ == "__main__":
    ...
    handler = lambda *args, **kwargs: AlphaOceanRequestHandler(
        *args, directory=str(project_root), **kwargs
    )
    server = ThreadingHTTPServer((HOST, PORT), handler)
    print(f"Alpha Ocean local server: http://{HOST}:{PORT}")
