"""Alpha local development server (Casino Style Theme on Port 5503)."""
...
class AlphaCasinoRequestHandler(SimpleHTTPRequestHandler):
...
if __name__ == "__main__":
    ...
    handler = lambda *args, **kwargs: AlphaCasinoRequestHandler(
        *args, directory=str(project_root), **kwargs
    )
    server = ThreadingHTTPServer((HOST, PORT), handler)
    print(f"Alpha Casino local server: http://{HOST}:{PORT}")
